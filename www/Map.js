...
var Map = function (__pgmId, _exec) {
    ...
    self.on('active_marker_changed', function (prevMarker, newMarker) {
        ...
        self.exec.call(self, null, null, self.__pgmId, 'setActiveMarkerId', [
            newMarkerId,
        ]);
    });
};
...
Map.prototype.getMap = function (meta, div, options) {
    ...
    if (options.controls) {
        ...
        this.set(
            'myLocationButton',
            options.controls.myLocationButton === true
        );
    }

    if (options.preferences && options.preferences.gestureBounds) {
        if (
            utils.isArray(options.preferences.gestureBounds) ||
            options.preferences.gestureBounds.type === 'LatLngBounds'
        ) {
            options.preferences.gestureBounds = common.convertToPositionArray(
        ...
        function () {
            ...
            if (common.isDom(div)) {
                ...
                if (self._layers.info.parentNode) {
                    try {
                        self._layers.info.parentNode.removeChild(
                            self._layers.info.parentNode
                        );
                    } catch (e) {
                        // ignore
                    }
...
Map.prototype.setOptions = function (options) {
    ...
    this.exec.call(this, null, this.errorHandler, this.__pgmId, 'setOptions', [
        options,
    ]);
    return this;
};

Map.prototype.getMyLocation = function (
    params,
    success_callback,
    error_callback
) {
    return window.plugin.google.maps.LocationService.getMyLocation.call(
        this,
        params,
        success_callback,
        error_callback
...
Map.prototype.setVisible = function (isVisible) {
    ...
    self.exec.call(self, null, self.errorHandler, this.__pgmId, 'setVisible', [
        isVisible,
    ]);
    return this;
};
...
Map.prototype.getVisibleRegion = function (callback) {
    ...
    if (typeof callback === 'function') {
        console.log(
            '[deprecated] getVisibleRegion() is changed. Please check out the https://goo.gl/yHstHQ'
        );
...
Map.prototype.setPadding = function (p1, p2, p3, p4) {
    ...
    this.exec.call(this, null, self.errorHandler, this.__pgmId, 'setPadding', [
        padding,
    ]);
    return this;
};
...
Map.prototype.addKmlOverlay = function (kmlOverlayOptions, callback) {
    ...
    kmlOverlayOptions.clickable = common.defaultTrueOption(
        kmlOverlayOptions.clickable
    );
    kmlOverlayOptions.suppressInfoWindows =
        kmlOverlayOptions.suppressInfoWindows === true;

    if (kmlOverlayOptions.url) {
        var link = document.createElement('a');
        link.href = kmlOverlayOptions.url;
        kmlOverlayOptions.url =
        ...
        var resolver = function (resolve, reject) {
            ...
            loader.parseKmlFile(function (camera, kmlData) {
                ...
                var kmlId =
                    'kmloverlay_' + Math.floor(Math.random() * Date.now());
                var kmlOverlay = new KmlOverlay(
                    self,
                    kmlId,
                    camera,
                    kmlData,
...
Map.prototype.addGroundOverlay = function (groundOverlayOptions, callback) {
    ...
    groundOverlayOptions.bearing =
        'bearing' in groundOverlayOptions ? groundOverlayOptions.bearing : 0;
    groundOverlayOptions.url = groundOverlayOptions.url || null;
    groundOverlayOptions.clickable = groundOverlayOptions.clickable === true;
    groundOverlayOptions.visible = common.defaultTrueOption(
        groundOverlayOptions.visible
    );
    groundOverlayOptions.zIndex = groundOverlayOptions.zIndex || 0;
    groundOverlayOptions.bounds = common.convertToPositionArray(
        groundOverlayOptions.bounds
    );
    ...
    self.exec.call(
        ...
        self.__pgmId,
        'loadPlugin',
        ['GroundOverlay', groundOverlayOptions, groundOverlay.hashCode]
    );
...
Map.prototype.addTileOverlay = function (tilelayerOptions, callback) {
    ...
    if (typeof tilelayerOptions.tileUrlFormat === 'string') {
        console.log(
            '[deprecated] the tileUrlFormat property is now deprecated. Use the getTile property.'
        );
        tilelayerOptions.getTile = function (x, y, zoom) {
            return tilelayerOptions.tileUrlFormat
                .replace(/<x>/gi, x)
                .replace(/<y>/gi, y)
    ...
    tilelayerOptions.visible = common.defaultTrueOption(
    ...
    tilelayerOptions.opacity =
        tilelayerOptions.opacity === null ||
        tilelayerOptions.opacity === undefined
            ? 1
            : tilelayerOptions.opacity;
    tilelayerOptions.debug = tilelayerOptions.debug === true;
    tilelayerOptions.userAgent =
        tilelayerOptions.userAgent || navigator.userAgent;
    ...
    tileOverlay.one(tileOverlayId + '_remove', function () {
        document.removeEventListener(
            tileOverlayId + '-' + hashCode + '-tileoverlay',
            onNativeCallback
        );
        tileOverlay.off();
    ...
    var onNativeCallback = function (params) {
        ...
        if (url instanceof Promise) {
            ...
            cordova_exec(
                ...
                self.__pgmId + '-tileoverlay',
                'onGetTileUrlFromJS',
                [hashCode, params.key, url]
            );
        }
    };
    document.addEventListener(
        self.__pgmId + '-' + hashCode + '-tileoverlay',
        onNativeCallback
    );
...
Map.prototype.addPolygon = function (polygonOptions, callback) {
    ...
    polygonOptions.points = common.convertToPositionArray(
        polygonOptions.points
    );
    polygonOptions.holes = polygonOptions.holes || [];
    if (
        polygonOptions.holes.length > 0 &&
        !Array.isArray(polygonOptions.holes[0])
    ) {
        polygonOptions.holes = [polygonOptions.holes];
    }
    ...
    polygonOptions.strokeColor = common.HTMLColor2RGBA(
        polygonOptions.strokeColor || '#FF000080',
        0.75
    );
    if (polygonOptions.fillColor) {
        polygonOptions.fillColor = common.HTMLColor2RGBA(
            polygonOptions.fillColor || '#FF000080',
            0.75
        );
    } else {
    ...
    polygonOptions.strokeWidth =
        'strokeWidth' in polygonOptions ? polygonOptions.strokeWidth : 10;
...
Map.prototype.addPolyline = function (polylineOptions, callback) {
    ...
    polylineOptions.points = common.convertToPositionArray(
        polylineOptions.points
    );
    polylineOptions.color = common.HTMLColor2RGBA(
        polylineOptions.color || '#FF000080',
        0.75
    );
    polylineOptions.width =
        'width' in polylineOptions ? polylineOptions.width : 10;
    ...
    self.exec.call(
        ...
        self.__pgmId,
        'loadPlugin',
        ['Polyline', opts, polyline.hashCode]
    );
...
Map.prototype.addCircle = function (circleOptions, callback) {
    ...
    circleOptions.strokeColor = common.HTMLColor2RGBA(
        circleOptions.strokeColor || '#FF0000',
        0.75
    );
    circleOptions.fillColor = common.HTMLColor2RGBA(
        circleOptions.fillColor || '#000000',
        0.75
    );
    circleOptions.strokeWidth =
        'strokeWidth' in circleOptions ? circleOptions.strokeWidth : 10;
    ...
    self.exec.call(
        ...
        self.__pgmId,
        'loadPlugin',
        ['Circle', circleOptions, circle.hashCode]
    );
...
Map.prototype.addMarker = function (markerOptions, callback) {
    ...
    if (
        typeof markerOptions.anchor === 'object' &&
        'x' in markerOptions.anchor &&
        'y' in markerOptions.anchor
    ) {
    ...
    self.exec.call(
        ...
        function (result) {
            if (marker) {
                if (!isBase64Marker) {
                    ...
                    markerOptions.icon.size.width =
                        markerOptions.icon.size.width || result.width;
                    markerOptions.icon.size.height =
                        markerOptions.icon.size.height || result.height;
                    markerOptions.icon.anchor = markerOptions.icon.anchor || [
                        markerOptions.icon.size.width / 2,
                        markerOptions.icon.size.height,
                    ];
                }
        ...
        self.__pgmId,
        'loadPlugin',
        ['Marker', markerOptions, marker.hashCode]
    );
...
Map.prototype.addMarkerCluster = function (markerClusterOptions, callback) {
    ...
    var markerCluster = new MarkerCluster(
        self,
        {
            ...
            boundsDraw: common.defaultTrueOption(
                markerClusterOptions.boundsDraw
            ),
        },
        exec
    );
...
Map.prototype._onClusterEvent = function (
    eventName,
    markerClusterId,
    clusterId,
    position
    ...
    if (markerCluster) {
        if (/^marker_/i.test(clusterId)) {
            ...
            if (cluster) {
                ...
                console.log(
                    '-----> This is remained cluster icon : ' + clusterId
                );
            }
        }
    }
